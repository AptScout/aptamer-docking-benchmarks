# An RNA Aptamer–Ligand Docking Benchmark with 49 Crystallographically Validated Cases
...
*[Figure: 47-case AUC bar chart showing aptamer scoring (blue/neutral/red) vs baseline (grey). 22/47 help (dark blue), 15/47 neutral (light blue), 10/47 hurt (red). Mean ΔAUC = +0.093.]*
...
- `enrich/`: active/decoy PDBQT sets (47/49 cases)
...
The benchmark serves a specific role in the modern ML-augmented drug discovery pipeline: **final validation** for RNA-targeted VS campaigns, providing physics-based enrichment after embedding-based pre-filtering (Milvus/Qdrant) and GNN-based rescoring. The critical gap this fills is that no GNN model trained on PDBbind protein-ligand data generalises to RNA-ligand binding (RNA is structurally and compositionally distinct from protein). AptScout's aptamer scoring terms — validated here across 47 diverse RNA aptamer structures — provide the only benchmarked enrichment signal for this target class.
...
```
manifest.json           — master case list (49 entries)
...
cases/
  {case-id}/
    ...
    enrich/             — present for 47/49 cases
      actives.pdbqt
      decoys.pdbqt
      crystal.pdbqt
...
**SAM fold trifecta.** SAM-I (+0.233), SAM-II (+0.127), and SAM-III (+0.163) all show positive aptamer scoring benefit despite three completely different RNA architectures. This confirms that the phosphate-electrostatic and stacking aptamer terms detect the SAM pharmacophore regardless of how the RNA pocket is arranged — the benefit is ligand-driven, not fold-driven, for SAM.

**Strong new hurt case.** The 2'-deoxyguanosine riboswitch (3SKI, ΔAUC = −0.300) is the strongest hurt case in the benchmark. The free deoxy-purine base has no phosphate and relies on H-bond selectivity; the decoy set contains guanine-like compounds that the stacking term rewards, mirroring the PreQ1 contamination pattern. The 3SKI case adds a third distinct mechanism for aptamer scoring failure (after FMN aromatic contamination and PreQ1 N-heterocyclic contamination).
...
Version used in this paper: v1.2.0-alpha (49 cases validated, 47 enrichment sets)
...
### Table 2. Complete Benchmark Results (47 evaluated cases)
...
| *New cases (batch 2)* | | | | | |
| 3',3'-cGAMP (4YAZ) | 0.184 | **0.541** | **+0.357** | — | — |
| SAM-III (3E5C) | 0.480 | **0.643** | **+0.163** | — | — |
| ppGpp (6DMC) | 0.295 | **0.504** | **+0.209** | — | — |
...
| *New cases (batch 4)* | | | | | |
| Glutamine (5DDQ) | 0.107 | **0.352** | **+0.245** | — | — |
| iMango-III (6E8S) | 0.483 | 0.550 | +0.067 | — | — |
| ZMP-II (4ZNP) | 0.493 | 0.373 | **−0.120** | — | — |
| SAM-V (6FZ0) | 0.567 | 0.423 | **−0.144** | — | — |
| FMN-inhibitor (5C45) | 0.095 | **1.000** | **+0.905** ★★ | **100×** | — |

★ = near-native pose (RMSD < 2 Å)
| *New cases (batch 5)* | | | | | |
| FMN-inhibitor-2 (5KX9) | 0.119 | **1.000** | **+0.881** ★★ | **100×** | — |
| Adenine+Mg (4TZX) | 0.876 | 0.743 | **−0.133** | — | — |
| 5-HTP aptamer (5KPY) | 0.551 | 0.483 | −0.068 | — | — |
| Guanidine-ykkC (5U3G) | 0.510 | 0.482 | −0.029 | — | — |
| iSpinach (5OB3) | 0.300 | 0.100 | **−0.200** | — | — |

| *New cases (batch 6)* | | | | | |
| Squash (7KVV) | 0.500 | **0.900** | **+0.400** | — | — |
| Pepper-HBC599 (7U0Y) | 0.167 | **0.400** | **+0.233** | — | — |
| thiM-TPP-analog (4NYA) | 0.507 | **0.578** | +0.070 ★★ | — | — |
| guanine-xanthine (3GAO) | 0.991 | 0.967 | −0.024 | — | — |
| Pepper-HBC (7SZU) | 0.633 | 0.283 | **−0.350** | — | — |

★ = near-native pose (RMSD < 2 Å)
★★ = synthetic drug discovery compound (not natural ligand)

### Summary Statistics

- **AUC improves (Δ > +0.05): 22/47 cases**  
- **AUC neutral (|Δ| ≤ 0.05): 15/47** (ceiling or near-random baseline)  
- **AUC worsens (Δ < −0.05): 10/47** (PreQ1 type I, Corn, guanine riboswitch, 2'-deoxyguanosine — decoy design or free-base/deoxy-purine ligand)  
- **Mean ΔAUC: +0.093**; Median ΔAUC: +0.050
...
- **EF₁% > 0: 22/47 cases** (active ranked in top 1% of pool; 3MXH achieves EF₁% = 50×)  
...
*47case-master.csv: all 47 evaluated cases, AUC + EF₁% + RMSD*

# An RNA Aptamer–Ligand Docking Benchmark with 44 Crystallographically Validated Cases
...
*[Figure: 42-case AUC bar chart showing aptamer scoring (blue/neutral/red) vs baseline (grey). 19/42 help (dark blue), 14/42 neutral (light blue), 9/42 hurt (red). Mean ΔAUC = +0.096.]*
...
- `enrich/`: active/decoy PDBQT sets (42/44 cases)
...
The benchmark serves a specific role in the modern ML-augmented drug discovery pipeline: **final validation** for RNA-targeted VS campaigns, providing physics-based enrichment after embedding-based pre-filtering (Milvus/Qdrant) and GNN-based rescoring. The critical gap this fills is that no GNN model trained on PDBbind protein-ligand data generalises to RNA-ligand binding (RNA is structurally and compositionally distinct from protein). AptScout's aptamer scoring terms — validated here across 42 diverse RNA aptamer structures — provide the only benchmarked enrichment signal for this target class.
...
```
manifest.json           — master case list (44 entries)
...
cases/
  {case-id}/
    ...
    enrich/             — present for 42/44 cases
      actives.pdbqt
      decoys.pdbqt
      crystal.pdbqt
...
Version used in this paper: v1.1.0-alpha (44 cases validated, 42 enrichment sets)
...
### Table 2. Complete Benchmark Results (42 evaluated cases)
...
- **AUC improves (Δ > +0.05): 19/42 cases**  
- **AUC neutral (|Δ| ≤ 0.05): 14/42** (ceiling or near-random baseline)  
- **AUC worsens (Δ < −0.05): 9/42** (PreQ1 type I, Corn, guanine riboswitch, 2'-deoxyguanosine — decoy design or free-base/deoxy-purine ligand)  
- **Mean ΔAUC: +0.096**; Median ΔAUC: +0.050
...
- **EF₁% > 0: 19/42 cases** (active ranked in top 1% of pool; 3MXH achieves EF₁% = 50×)  
...
*42case-master.csv: all 42 evaluated cases, AUC + EF₁% + RMSD*

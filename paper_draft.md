...
*[Figure: 50-case AUC bar chart showing aptamer scoring (blue/neutral/red) vs baseline (grey). 23/50 help (dark blue), 17/50 neutral (light blue), 10/50 hurt (red). Mean ΔAUC = +0.094.]*
...
- `enrich/`: active/decoy PDBQT sets (50/52 cases)
...
The benchmark serves a specific role in the modern ML-augmented drug discovery pipeline: **final validation** for RNA-targeted VS campaigns, providing physics-based enrichment after embedding-based pre-filtering (Milvus/Qdrant) and GNN-based rescoring. The critical gap this fills is that no GNN model trained on PDBbind protein-ligand data generalises to RNA-ligand binding (RNA is structurally and compositionally distinct from protein). AptScout's aptamer scoring terms — validated here across 50 diverse RNA aptamer structures — provide the only benchmarked enrichment signal for this target class.
...
```
manifest.json           — master case list (52 entries)
...
cases/
  {case-id}/
    ...
    enrich/             — present for 50/52 cases
      actives.pdbqt
      decoys.pdbqt
      crystal.pdbqt
...
| *New cases (batch 7)* | | | | | |
| PRPP (6CK5) | 0.151 | **0.476** | **+0.325** | — | — |
| purine-DAP (2XO0) | 0.943 | 0.971 | +0.028 | — | — |
| M6 (4LX5) | 0.924 | 0.881 | −0.043 | — | — |
...
- **AUC improves (Δ > +0.05): 23/50 cases**  
- **AUC neutral (|Δ| ≤ 0.05): 17/50** (ceiling or near-random baseline)  
- **AUC worsens (Δ < −0.05): 10/50** (PreQ1 type I, Corn, guanine riboswitch, 2'-deoxyguanosine — decoy design or free-base/deoxy-purine ligand)  
- **Mean ΔAUC: +0.094**; Median ΔAUC: +0.050
...
- **EF₁% > 0: 23/50 cases** (active ranked in top 1% of pool; 3MXH achieves EF₁% = 50×)  
...
*50case-master.csv: all 50 evaluated cases, AUC + EF₁% + RMSD*

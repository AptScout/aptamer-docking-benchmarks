...
The current MacVina 10-case reference-pose metrics are summarized in [`macvina_10_case_snapshot.md`](macvina_10_case_snapshot.md).
...
The `inputs/` files are intentionally ignored by git, but they exist locally for the current benchmark workspace. The current `reference_pose.pdbqt` files mirror the crystallographic ligand PDBQT and are exposed through `out = reference_pose.pdbqt` so MacVina and MacDock can parse a mode-1 reference pose.

Compatibility note: the local `2ygh-sam-riboswitch` ligand/reference PDBQT records use residue label `SAM A  96` instead of the source-style packed `SAM A1096`, because the latter overflows fixed-column PDBQT residue parsing in MacVina.

## MacDock Snapshots

Claude initially reported the following MacDock processing snapshot:
...
Claude traced that flat delta to placeholder benchmark-path scoring in MacDock and reports that it has been replaced with pose-geometry-based grid scoring. See [`macdock_aptamer_scoring_audit.md`](macdock_aptamer_scoring_audit.md).

Post-fix, the current reported MacDock aptamer-term breakdown is case-specific:
...
- Treat `2g9c-purine-riboswitch` as pending contact review for MacVina/MacDock stacking calibration; its local close contacts do not currently produce a MacVina base-stacking term.
...
- Use the panel to check whether MacVina and MacDock read expected contacts and compute case-specific aptamer terms.
...
2. Add per-case pose reports from MacVina and MacDock once both tools emit case-specific aptamer term breakdowns.

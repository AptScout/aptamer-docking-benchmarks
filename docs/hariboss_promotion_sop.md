...
This procedure converts a tracked HARIBOSS seed directory into a runnable AptScout/AptScout benchmark case. The goal is consistency: every promoted case should differ because of RNA-ligand biology, not because of ad-hoc preparation.
...
| Validation | `python3 scripts/validate_manifest.py` and AptScout `--validate-only` both pass after adding the case |
...
## AptScout Calibration Notes

- Keep `metadata_only` seeds outside `manifest.json`; AptScout currently treats manifest entries as runnable.
...
8. Run AptScout validate-only against the manifest.
